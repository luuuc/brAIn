#!/bin/sh
# install.sh — install the brAIn binary from GitHub Releases.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/luuuc/brain/main/install.sh | sh
#
# Installs to /usr/local/bin/brain (or ~/.local/bin/brain if /usr/local/bin
# is not writable). Verifies the SHA256 checksum of the downloaded archive.
...
echo "Fetching latest brAIn release..."
...
# Match the exact filename in field 2, not a substring — future artifacts
# that share a prefix (e.g. .sig / .asc siblings) must not match here.
...
mv "${WORK_DIR}/brain" "${INSTALL_DIR}/brain"
chmod +x "${INSTALL_DIR}/brain"

echo ""
echo "brAIn ${VERSION} installed to ${INSTALL_DIR}/brain"
